...
void insert(int x) {
	...
	int result = search(temp);				// 새로 받은 수를 담고 있는 주소 포인터를 서치함수에 대입
...
int search(node* temp) {
	...
	queue = head;
	if (queue->data > temp->data && queue == head) {		
		head = temp;
		temp->next = queue;
		return 1;
	}
	// 이 조건문을 while문 안에 넣게 되면 값이 처음 하나 입력되었을 때, queue의 next값이 NULL이므로 반복이 종료가 되버려 문제가 발생한다.
	// 따라서 조건문을 실행하기 전에 입력함으로써 이 문제를 해결한다.

	while (queue->next != NULL) {	// 마지막 queue->next가 tail이고 tail이 NULL이면 반복문 종료
		
		if (queue->data < temp->data && queue->next->data > temp->data) {
			temp->next = queue->next;
			queue->next = temp;
			return 1;
		}
			
		queue = queue->next;		// 주소를 head -> V1 -> V2...로 변경해가며 판단
	}
	return 0;
}
